...
class EGLCore {
    ...
    internal fun getContext(): EGLContext {
        return mEGLContext
    }
     ...
     * @param version EGL版本
     */
    internal fun getConfig(flags: Int, version: Int): EGLConfig? {
        ...
        val attrList = intArrayOf(
            ...
            EGL14.EGL_SURFACE_TYPE, EGL_WINDOW_BIT or EGL_PBUFFER_BIT,
    ...
    fun makeCurrent(eglSurface: EGLSurface) {
        ...
        if (!EGL14.eglMakeCurrent(mEGLDisplay, eglSurface, eglSurface, mEGLContext)) {
            throw RuntimeException("makeCurrent(eglSurface) failed ${checkEglError("makeCurrent")}")
        }
    }
    ...
    fun makeCurrent(drawSurface: EGLSurface, readSurface: EGLSurface) {
        ...
        if (!EGL14.eglMakeCurrent(mEGLDisplay, drawSurface, readSurface, mEGLContext)) {
            throw RuntimeException("eglMakeCurrent(draw,read) failed ${checkEglError("makeCurrent")}")
        }
    }
    ...
    private fun checkEglError(msg: String): String {
        var error: Int
        if (EGL14.eglGetError().also { error = it } != EGL14.EGL_SUCCESS) {
            return "$msg: EGL error: 0x${Integer.toHexString(error)}"
        }
        return ""
    }
}