...
void main()
{
    vec4 sourceColor = texture2D(u_TextureUnit, v_TextureCoordinates);
    ...
    for (int i = 1; i < uBlurRadius; i++) {
        weight = getWeight(i);
        finalColor += texture2D(u_TextureUnit, clampCoordinate(v_TextureCoordinates - uBlurOffset * float(i))).rgb * weight;
        finalColor += texture2D(u_TextureUnit, clampCoordinate(v_TextureCoordinates + uBlurOffset * float(i))).rgb * weight;
    }
