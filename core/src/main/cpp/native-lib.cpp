...
Java_com_maverick_multimedia_camera_face_FaceTracker_create(JNIEnv *env, jobject thiz,
...
Java_com_maverick_multimedia_camera_face_FaceTracker_start(JNIEnv *env, jobject thiz, jlong self) {
    if (self == 0) {
        return;
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_maverick_multimedia_camera_face_FaceTracker_stop(JNIEnv *env, jobject thiz, jlong self) {
    if (self == 0) {
        return;
    }
    FaceTracker *me = (FaceTracker *) self;
...
Java_com_maverick_multimedia_camera_face_FaceTracker_detector(JNIEnv *env, jobject thiz, jlong self,
                                                              jbyteArray data_, jint lensFacing, jint width, jint height) {
    ...
    // 颜色格式的转换 nv21->RGBA
    // 将 nv21的yuv数据转成了rgba
    cvtColor(src, src, COLOR_YUV2GRAY_NV21);
    // 正在写的过程 退出了，导致文件丢失数据
...
//        rotate(src, src, ROTATE_90_CLOCKWISE);
    ...
    src.release();
    int ret = rects.size();
    LOGD("ret: %d (%d, %d)", ret, w, h);
    if (ret) {
        jclass clazz = env->FindClass("com/maverick/multimedia/camera/face/Face");
        jmethodID constructor = env->GetMethodID(clazz, "<init>", "(IIII[F)V");
        int size = ret * 2;
        //创建java 的float 数组
        ...
        jobject face = env->NewObject(clazz, constructor, width, height, w, h, floatArray);
        return face;
    }
    return 0;
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_maverick_multimedia_camera_face_FaceTracker_saveYUV(JNIEnv *env, jclass clazz, jbyteArray yuv) {
